...
import { reserveStockSchema, variantIdParamSchema } from '@/modules/inventory/validators';
import {
  apiSuccess,
  apiUnauthorized,
  apiValidationError,
...
export async function POST(request: Request, context: RouteContext) {
  try {
    ...
    const parsedBody = reserveStockSchema.safeParse(body);

    if (!parsedBody.success) {
      return apiValidationError(parsedBody.error);
    }

    const result = await inventoryServerService.reserveStock(
      user.id,
