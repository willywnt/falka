...
`check_design_system` (the claude.ai/design SERVER self-check) scans the compiled `_ds_bundle.css`
for custom-property **declarations** (`--x: value;`) and warns on any it can't classify. The fix took
three runs to land; the mechanics below are the hard-won truth — trust them over the original triage.

- **WHAT it scans**: actual `--x: value;` DECLARATIONS, wherever they appear (`:root`/`@theme`, the
  universal `@supports … { *,::before,… { --tw-…: …; } }` reset block, AND utility-class bodies). It
  does NOT read `@property --tw-*` REGISTRATIONS as tokens — those, and their `syntax`/`inherits`/
  `initial-value` descriptors, are not custom-property declarations. (Run 2 wasted effort annotating
  `@property` blocks; the checker ignored every one.)
- **HOW to classify**: an inline `/* @kind … */` is honored ONLY when it sits on a declaration, right
  after its `;` (`--tw-translate-x: 0; /* @kind other */`). On a selector line or `@property` line it
  is silently ignored.
- **FIXED durably (2026-06-22, run 3)**: `build-inputs.mjs` §3b post-processes the compiled CSS to
  append `/* @kind other */` to (a) the 8 motion/timing `@theme` tokens and (b) **every** `--tw-*: …;`
  declaration (~237 — reset block + utility classes). None are color/spacing/radius/shadow/font design
  tokens. Build prints `css: annotated N declarations @kind other` (≈245). `@property` rules are left
  untouched. The regex is `/(--tw-[\w-]+:[^;{}]*;)/g` (`[^;{}]*` keeps it inside one declaration).
- **WHY annotate, not "exclude"**: the notes asked to "exclude `--tw-*` from token scanning via the
  sync config" — there is **NO such knob**. Token scanning is server-side; `package-build.mjs` emits
  only the element `replaces` map, never a token list (the `_adherence.oxlintrc.json`
  `x-omelette.tokens`/`tokenKinds` block is regenerated by the server from the CSS). Annotating every
  `--tw-*` declaration as `other` IS the realization of "treat all `--tw-*` as non-tokens".
- It MUST live on the GENERATED output: the source `@theme` block has no comments and the `--tw-*`
  rules are emitted by Tailwind. Don't move `--tw-*` into `:root` (breaks Tailwind utilities).
