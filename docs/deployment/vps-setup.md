...
#   APP_IMAGE=palka-app:prod   WEB_ALIAS=palka-prod-web
...
- `APP_IMAGE` + `WEB_ALIAS` — distinct per env (`palka-app:prod`/`palka-prod-web` vs `:staging`/`palka-staging-web`).
...
docker compose -p palka-prod --env-file .env.production run --rm \
  ...
  migrate pnpm --filter @palka/db db:bootstrap-admin
```
...
Restore: `gunzip -c db-YYYYMMDD.sql.gz | docker compose -p palka-prod exec -T postgres psql -U palka -d palka`.
...
cp .env.production.vps.example .env.production     # APP_IMAGE=palka-app:staging,
...
- **Brand** — the codebase was renamed **Falka → Palka** (`@palka/*`, `palka-app`) on 2026-06-28.
  Trademark clearance is still in progress (Falka was the prior fallback) — see `docs/business/`.
