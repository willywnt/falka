...
| `INTERNAL_API_SECRET`           | Prod     | Dedicated secret (≥32 chars, distinct from `AUTH_SECRET`) for the loopback internal endpoints (scheduled order pull + finance auto-gen). In prod the endpoints **refuse (503)** if unset — they never fall back to `AUTH_SECRET`. Dev falls back. |
...
| `ORDERS_AUTO_PULL_INTERVAL_MS`  | No       | Custom-server scheduled Lazada order-pull interval in ms; unset/0 = off. Set (e.g. 300000) once a real shop is connected                                                                                                                          |
| `FINANCE_AUTOGEN_ENABLED`       | No       | `true` = custom server materializes recurring opex + fees on the 1st (UTC); off by default                                                                                                                                                        |
