...
4. **Honest + boundary-respecting.** The bell keeps its exact contract; the derived live signals stay (they
   drop to 0 the instant stock is fixed). Notifications are **cross-cutting** → a shared `lib/`, not a module
   (every module would otherwise import a notifications module = boundary break / cycles).
...
| ----------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------- |
| **Sync-in-tx** (one upsert in the tx that already mutates the entity)         | `ORDER_PLACED`/`ORDER_SHIPPED` (`orders-server.service.ts` `pullOneConnection` ~514/605, guarded by `inventoryAppliedAt`/`inventoryShippedAt`), `RETURN_PROCESSED` (`returns-server.service.ts` `processReturn`), `SALE_REFUNDED`+`SALE_BELOW_COST` (`sales-server.service.ts` `createSale`/refund), `PURCHASE_RECEIVED` (`purchasing-server.service.ts` `receivePurchaseOrder`), `OPNAME_POSTED` (`stock-opname.service.ts` `postOpname`) | producer in the existing service tx                                                                                                    | ✅ works now                                                               |
...
| **1 — Persistent log + server read-state** | `Notification`+`NotificationRead` migration; `lib/notifications/{notification-server.service,notification.repository,notification-keys}`; `GET /api/v1/notifications` + `POST /read-all` + `POST /:id/read` (withApiRoute, Zod, one service); `use-notifications` becomes the UNION selector; read state moves to DB (localStorage dual-read this phase only). Bell unchanged. **100% Vercel-native, shares NONE of WhatsApp's gating.** | schema                                          | ~3–4d                           |
| **2 — Producers**                          | Discrete in-tx producers + low-stock/oversold crossing detection + the Vercel-Cron recompute for rolled-up signals; retire `notifications-store.ts` (server-only read state); retention cleanup (Vercel Cron, ~90d, cascades reads)                                                                                                                                                                                                      | none                                            | ~4–5d                           |
...
_Recommended first executable step when we resume: **Phase 1** (it's the load-bearing, Vercel-native piece and
needs only the `Notification`+`NotificationRead` migration). Confirm decisions 1–3 first._
