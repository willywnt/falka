...
| 4   | **Lazada multi-warehouse stock sync** (KNOWN GAP)                    | marketplace       | M      | 🟡   | A Lazada SKU can hold stock across several warehouses (`multiWarehouseInventories[]`). Our push (`/product/stock/sellable/update` with the bare `<SellableQuantity>`) sets only ONE (default/last), so a multi-warehouse SKU's external total ≠ internal available and drift never clears. Single-warehouse / dropshipping sellers are fine. **Fix:** capture `warehouseCode` per SKU at import; decide the policy (set a designated/sync warehouse to internal + zero the rest? a per-connection "sync warehouse" config?); build the `<MultiWarehouseInventories><WarehouseCode>…</WarehouseCode><SellableQuantity>…` payload; make drift compare the right external value (sum vs the synced warehouse). Likely needs a small migration for the per-connection/per-listing warehouse choice → product decision first. Found 2026-06-16. |
