...
- **Lazada Open Platform — real OAuth multi-seller integration** (branch `session/lazada-integration`,
  ...
  `/product/stock/sellable/adjust` (POST, absolute `SellableQuantity`), the path dropshipping-warehouse
  sellers can call; live-validated `code:0`. The scheduled refresh worker also shipped (Phase 6).
...
| C   | **Real Shopee / Tokopedia / TikTok adapters**                               | L (each) | 🔴   | **Lazada is now REAL + OAuth-onboarded + live-validated** (import + **stock-WRITE both work** — write via `/product/stock/sellable/adjust`, 2026-06-15). Shopee/Tokopedia/TikTok each still need OAuth + signed client + import + stock-sync + webhook/poll — reuse the Lazada OAuth pattern. **Start Shopee partner paperwork now — 6–12 wk lead time.** |
